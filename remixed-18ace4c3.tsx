...
import { Upload, FileText, Loader2, FileJson, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronUp, ChevronDown, Settings2, GripVertical } from 'lucide-react';
...
const flattenJson = (json) => {
  ...
  return a.map(item => {
    const row = {};
    const walk = (o, p) => { for (const [k, v] of Object.entries(o || {})) { const key = p ? `${p}.${k}` : k; if (v && typeof v === 'object' && !Array.isArray(v)) walk(v, key); else row[key] = Array.isArray(v) ? v.join(', ') : v; } };
...
const rowsToCsv = (rows) => {
  ...
  return [h.join(','), ...rows.map(r => h.map(k => { const s = String(r[k] ?? ''); return s.includes(',') || s.includes('"') || s.includes('\n') ? `"${s.replace(/"/g, '""')}"` : s; }).join(','))].join('\n');
};
const parseCsv = (text) => {
...
function CellValue({ value }) {
  ...
  return (<span>{s.split(re).map((part, i) =>
    re.test(part)
      ? <a key={i} href={part} target="_blank" rel="noopener noreferrer" style={{ color: '#4f46e5', textDecoration: 'underline' }}>{part}</a>
      : part
  )}</span>);
}
...
function Pagination({ total, page, size, onPage, onSize }) {
  ...
  const e = Math.min(page * eff, total);
  const info = isJa
  ...
  const B = ({ onClick, disabled, children }) => (
    <button onClick={onClick} disabled={disabled} style={{ padding: 4, borderRadius: 4, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.3 : 1, background: 'none', border: 'none' }}>{children}</button>
  );
  return (
...
function ColPanel({ cols, onToggle, onToggleAll, onReorder, onReset, onClose }) {
  ...
  return (
    <div style={{ position: 'absolute', right: 0, top: 32, zIndex: 50, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,.1)', padding: 12, minWidth: 220, maxHeight: 320, overflowY: 'auto' }}
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
        {[['showAll', true], ['hideAll', false]].map(([k, v]) => (
          <button key={k} onClick={() => onToggleAll(v)} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, border: '1px solid #e5e7eb', background: '#f9fafb', cursor: 'pointer' }}>{t(k)}</button>
        ))}
...
function useColumnResize(initialWidths) {
  ...
  const onMouseDown = (colKey, e) => {
    ...
    const startW = widths[colKey] || 150;

    const onMove = (ev) => {
      const diff = ev.clientX - startX;
      const newW = Math.max(60, Math.min(600, startW + diff));
  ...
  const resetWidths = (keys) => {
    const w = {};
    keys.forEach(k => { w[k] = 150; });
    setWidths(w);
  };

  return { widths, onMouseDown, setWidths, resetWidths };
}

/* ── DataTable ── */
function DataTable({ headers, rows, isArrayRows }) {
  ...
  const initWidths = () => {
    const w = {};
    headers.forEach(h => { w[h] = 150; });
    return w;
  };
  const { widths, onMouseDown, resetWidths } = useColumnResize(initWidths());

  // Reset if headers change
  const prevH = useRef(headers);
  if (JSON.stringify(headers) !== JSON.stringify(prevH.current)) {
    prevH.current = headers;
    setCols(headers.map(h => ({ key: h, visible: true })));
    resetWidths(headers);
    setSortKey(null); setPg(1);
  }
  ...
  const pageRows = pgSize === 'all' ? sorted : sorted.slice((pg - 1) * eff, pg * eff);
  ...
  const reorder = (from, to) => {
    setCols(prev => { const a = [...prev]; const [item] = a.splice(from, 1); a.splice(to, 0, item); return a; });
  };
  ...
  const handleHoverStyle = '2px solid #a5b4fc';
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8, position: 'relative' }}>
        <button onClick={() => setPanel(!panel)}
          style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#6b7280', background: 'none', border: '1px solid #e5e7eb', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>
      ...
      <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: 8 }}>
        <table style={{ borderCollapse: 'collapse', tableLayout: 'fixed', width: visCols.reduce((s, c) => s + (widths[c.key] || 150), 0) }}>
          <colgroup>
            {visCols.map(col => (
          ...
          <thead>
            <tr>
              {visCols.map(col => (
                <th key={col.key}
                  style={{
                    padding: '8px 16px 8px 12px', textAlign: 'left', fontWeight: 600, fontSize: 12,
                  ...
                  <div
                    style={handleStyle}
                    onMouseEnter={(e) => { e.currentTarget.style.borderRight = handleHoverStyle; }}
              ...
              <tr key={ri} style={{ background: ri % 2 === 0 ? '#fff' : '#fafafa' }}>
                {visCols.map(col => (
                  <td key={col.key} style={{
                    ...
                    fontSize: 12, color: '#4b5563', verticalAlign: 'top',
                    overflow: 'hidden', width: widths[col.key] || 150,
                  }}>
                    <div style={{
      ...
      {total > 10 && (
        <Pagination total={total} page={pg} size={pgSize} onPage={setPg} onSize={setPgSize} />
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const loadFile = async (file) => {
    ...
    try {
      ...
      if (ext === 'json') {
        ...
        setCsvStr(rowsToCsv(rows));
      } else if (ext === 'csv') {
        const p = parseCsv(text);
        hdrs = p.h; rows = p.rows;
        setCsvStr(text);
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const fn = new Function('csvData', d.content[0].text + '\nreturn analysisResult;');
      setResult(fn(csvStr));
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f8fafc, #eef2ff 50%)', padding: 24 }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>{t('pageTitle')}</h1>
        <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 24 }}>{t('instructionsText')}</p>

        {/* Upload */}
        <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 112, border: '2px dashed #d1d5db', borderRadius: 12, background: '#fff', cursor: 'pointer' }}
        ...
        {fileInfo && (
          <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: '#6b7280' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 99, background: '#e0e7ff', color: '#4338ca', fontWeight: 500 }}>
              {fileInfo.type === 'JSON' ? <FileJson size={14} /> : <FileText size={14} />}{fileInfo.type}
            </span>
            <span>{fileInfo.name}</span>
            <span>•</span>
            <span>{fileInfo.rows.toLocaleString(locale)} {t('rows')} × {fileInfo.cols} {t('columns')}</span>
          </div>
        )}
        ...
        {allRows.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <h2 style={{ fontSize: 13, fontWeight: 600, color: '#4b5563', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={analyze} disabled={loading || !query.trim()}
                style={{ padding: '10px 20px', fontSize: 14, fontWeight: 500, color: '#fff', background: loading || !query.trim() ? '#d1d5db' : '#6366f1', border: 'none', borderRadius: 8, cursor: loading || !query.trim() ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>
